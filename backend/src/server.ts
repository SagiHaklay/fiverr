...
app.get('/:shortCode', async (req, res) => {
  try {
    ...
    await db.none(
      'UPDATE links SET access_count = access_count + 1 WHERE short_code = $1',
      [shortCode]
    );
...
app.post('/links', async (req, res) => {
  try {
    ...
    const existingLink = await db.oneOrNone(
      'SELECT short_code FROM links WHERE target_url = $1',
      [targetUrl]
    );
    ...
    await db.none(
      'INSERT INTO links(target_url, short_code) VALUES($1, $2)',
      [targetUrl, shortCode]
    );
...
app.get('/links/:shortCode/stats', async (req, res) => {
  try {
    ...
    const link = await db.oneOrNone(
      'SELECT target_url, short_code, created_at, access_count FROM links WHERE short_code = $1',
      [shortCode]
    );
